...
//   %%REG    CMSTR       16       16        0x00       0          W|R        TRUE        W   
//   %%REG    CMCR        16       16        0x02       0          W|R        TRUE        W   
//   %%REG    CMCNT       16       16        0x04       0          W|R        TRUE        W   
//   %%REG    CMCMSE      16       16        0x06       0xFFFF     W|R        TRUE        -   
//  
// 
// 
// 
...
// %REG_NAME  CMCNT
//   %%TITLE   name         upper     lower     init       access       support     callback    
//   %%BIT     CMCNT        15         0         0          W|R          TRUE           W    
// 
// 
// %REG_NAME CMCMSE
//   %%TITLE  name         upper  lower  init  access  support  callback
//   %%BIT    CMCMSE      15     0      0     R|W     TRUE     W
////////////////////////////////////////////////////////////////////////////////
/// @file cmt.h
/// @brief Register IF class of model CMT
/// $Id$
/// $Date$
/// $Revison$
/// $Author$
...
class Ccmt: public Ccmt_regif
          ...
          , public vpcl::tlm_tgt_if<32, tlm::tlm_base_protocol_types, 1>
{
#include "cmt_cmdif.h"
public:
    SC_HAS_PROCESS(Ccmt);
...
private:
    long int mfreq_CLK;//max 
    enum eInterruptControl
    {
      emInterrupt_enable = 1
    };
    ...
    double mPCLKPeriod;
    bool mIRQVal;
    bool mCMTReset;//CMT reset by preset_n
    bool mCmdReset;//CMT reset by handleCommand
    bool mIsCmdResetStatus;//Indicate handle command reset status
    ...
    void DumpInterruptMsg();

    //Register IF Callback Function
    void cb_CMSTR_STR0(RegCBstr str);
    void cb_CMSTR_STR1(RegCBstr str);
    void cb_CMCR_CKS(RegCBstr str);
    void cb_CMCNT_CMCNT(RegCBstr str);
    void cb_CMCMSE_CMCMSE(RegCBstr str);

    void tgt_acc (tlm::tlm_generic_payload &trans, sc_time &t);
    void irqChannelMethod();
    unsigned int tgt_acc_dbg(tlm::tlm_generic_payload &trans);

    //process interrupt
    void cmpMatchOutputHandling(bool value);//atch register count and COR 
    void interruptHandling();//handling interrupt, toogle interrupt
    //void DumpInterruptMsg();//dump  for cmd_cmdif.h
